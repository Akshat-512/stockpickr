...
# -*- coding: utf-8 -*-
"""
Enhanced Trading System for ICICI Direct Breeze API
Modular structure with improved stock selection for 10% monthly targets
"""
...
from app.breeze_api import BreezeAPIClient
from app.analysis.stock_screening import StockScreener
...
INDEX_NAME = config.get("Screening", "index_name", fallback="NIFTY 50")
PAPER_TRADING_MODE = config.getboolean("Strategy", "paper_trading_mode", fallback=True)

# Email settings
EMAIL_ENABLED = config.getboolean("Notifications", "enable_email", fallback=False)
...
# ============================================================================
# NOTIFICATION UTILITIES (Integrated)
# ============================================================================
...
def send_email(subject: str, body: str) -> bool:
    ...
    if (
        ...
        logging.debug("Email not configured or disabled")
        return False
...
def send_entry_notification(opportunity: Dict):
    """Send position entry notification"""
    subject = f"New Position: {opportunity['stock_code']}"

    body = f"""
ENTRY ALERT: {opportunity['stock_code']}

Details:
--------
Entry Price: ₹{opportunity['current_price']:.2f}
Stop Loss: ₹{opportunity['stop_loss']:.2f}
Signal: {opportunity['signal_reason']}
Relative Strength: {opportunity.get('relative_strength', 'N/A')}
Quality Score: {opportunity.get('quality_score', 'N/A')}

This position is targeting the monthly 10% goal.
Estimated holding period: 2-4 weeks
    """

    send_email(subject, body)


def send_exit_notification(stock_code: str, exit_details: Dict):
    """Send position exit notification"""
    subject = f"Position Closed: {stock_code}"

    pnl = exit_details.get("profit_loss", 0)
    pnl_pct = exit_details.get("profit_loss_percent", 0)
...
Details:
--------
Entry Price: ₹{exit_details.get('entry_price', 0):.2f}
Exit Price: ₹{exit_details.get('exit_price', 0):.2f}
P&L: ₹{pnl:.2f} ({pnl_pct:+.2f}%)
Exit Reason: {exit_details.get('exit_reason', 'Unknown')}
Days Held: {exit_details.get('holding_days', 0)}
...
The system will attempt to continue operation.
    """
...
def check_market_status() -> bool:
    ...
    # Paper trading mode - always open
    if PAPER_TRADING_MODE:
        logging.debug("Paper trading mode - market always open")
        return True
    ...
    if current_day not in trading_days:
        logging.info(f"Market closed: {current_day} is not a trading day")
        return False

    # Check trading hours
    if current_time < market_open or current_time > market_close:
        logging.info(f"Market closed: {current_time} is outside trading hours")
        return False
...
def get_index_stocks(index_name: str) -> List[Dict]:
    """Get stocks for a given index"""
    try:
        ...
        if os.path.exists(filepath):
            ...
            if "Symbol" in df.columns:
                ...
                logging.info(f"Loaded {len(stocks)} stocks from {filepath}")
                return stocks

        # Fallback to hardcoded lists
        logging.warning(f"Using hardcoded list for {index_name}")
        ...
        if index_name == "NIFTY 50":
            ...
            logging.warning(f"No data available for {index_name}")
            return []

        return [{"stock_code": code, "exchange_code": "NSE"} for code in stocks]
...
def create_directories():
    ...
    directories = ["logs", "reports", "positions", "trades", "index_csv"]
...
class TradingSystem:
    ...
    def __init__(self):
        ...
        self.api = BreezeAPIClient(config)
        self.screener = StockScreener(config, self.api)
        self.market_analyzer = MarketAnalyzer(self.api)
        self.portfolio = PositionManager(config, self.api)
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.api.initialize():
                logging.error("Failed to initialize API connection")
                return False
    ...
    def run_strategy(self) -> bool:
        ...
        try:
            logging.info("=== STARTING ENHANCED TRADING STRATEGY ===")

            # Check market status
            if not check_market_status():
                logging.info("Market is closed. Strategy execution skipped.")
                return True

            # Step 1: Analyze market conditions
            logging.info("Step 1: Analyzing market conditions...")
            market_condition = self.market_analyzer.get_enhanced_market_condition()

            if not market_condition:
                logging.error("Failed to analyze market conditions")
                return False

            self._log_market_condition(market_condition)

            # Step 2: Manage existing positions
            logging.info("Step 2: Managing existing positions...")
            positions_summary = self.portfolio.get_portfolio_summary()
            logging.info(f"Current positions: {positions_summary['positions_count']}")

            # Check for exits
            exited_count = self.portfolio.check_exit_signals(market_condition)
            logging.warning(f"Exited {exited_count} positions")

            # Update trailing stops
            updated_stops = self.portfolio.update_trailing_stops()
            logging.warning(f"Updated {updated_stops} trailing stops")

            # Step 3: Look for new opportunities
            logging.info("Step 3: Scanning for new opportunities...")

            can_enter_new = self._can_enter_new_position(
                market_condition, positions_summary
            )

            if can_enter_new:
                success = self._find_and_enter_opportunity(market_condition)
                if success:
                    logging.warning("Successfully entered new position")
                else:
                    logging.info("No suitable opportunities found")
            else:
                logging.info("Conditions not suitable for new positions")
    ...
    def get_session_token(self) -> str:
        return self.api.SESSION_TOKEN

    def _can_enter_new_position(
        self, market_condition: dict, positions_summary: dict
    ) -> bool:
        """Check if we can enter new positions"""

        # Check position limit
        if positions_summary["positions_count"] >= MAX_POSITIONS:
            logging.info(f"At maximum positions ({MAX_POSITIONS})")
            return False

        # Check market conditions - CRITICAL for monthly targets
        if not market_condition.get("above_200_sma", False):
            logging.info("Market below 200-day SMA - no new positions")
            return False

        market_quality = market_condition.get("market_quality", "poor")
        if market_quality not in ["good", "excellent"]:
            logging.info(f"Market quality insufficient: {market_quality}")
            return False

        return True

    def _find_and_enter_opportunity(self, market_condition: dict) -> bool:
        """Find and enter the best trading opportunity"""
        try:
            # Get expanded stock universe for better opportunities
            ...
            logging.info(f"Screening {len(stock_universe)} stocks for opportunities")

            if not stock_universe:
                logging.warning("No stocks in universe")
                return False

            # Get index data for relative strength calculations
            index_data = self.market_analyzer.get_index_data()

            # Find best opportunity with enhanced screening
            opportunity = self.screener.get_best_opportunity(
                stock_universe, market_condition, index_data, self.portfolio.positions
            )

            if not opportunity:
                logging.info("No high-quality opportunities found")
                return False

            # Log opportunity details
            logging.warning(f"Best opportunity: {opportunity['stock_code']}")
            logging.warning(f"Signal: {opportunity['signal_reason']}")
            logging.warning(f"Quality Score: {opportunity.get('quality_score', 'N/A')}")
            logging.warning(
                f"Relative Strength: {opportunity.get('relative_strength', 'N/A')}"
            )

            # Enter position
            success = self.portfolio.enter_position(opportunity)

            if success:
                # Send notification
                send_entry_notification(opportunity)
                return True
            else:
                logging.error(
                    f"Failed to enter position for {opportunity['stock_code']}"
                )
                return False

        except Exception as e:
            logging.error(f"Error finding opportunity: {e}")
            return False

    def _log_market_condition(self, market_condition: dict):
        """Log detailed market condition"""
        logging.warning("=== MARKET ANALYSIS ===")
        logging.warning(f"Trend: {market_condition.get('trend', 'unknown')}")
        logging.warning(
            f"Above 200-SMA: {market_condition.get('above_200_sma', False)}"
        )
        logging.warning(
            f"Market Quality: {market_condition.get('market_quality', 'unknown')}"
        )
        logging.warning(
            f"Momentum Direction: {market_condition.get('momentum_direction', 'unknown')}"
        )

        if "recent_5d_change" in market_condition:
            logging.warning(
                f"Recent 5-day change: {market_condition['recent_5d_change']:.2f}%"
            )

        if "close_vs_sma200" in market_condition:
            sma_diff = market_condition["close_vs_sma200"]
            logging.warning(f"Distance from 200-SMA: {sma_diff:+.2f}%")

    def _log_session_summary(self):
        """Log session summary"""
        summary = self.portfolio.get_portfolio_summary()

        logging.warning("=== SESSION SUMMARY ===")
        logging.warning(f"Active Positions: {summary['positions_count']}")
        logging.warning(f"Portfolio Value: ₹{summary['total_value']:.2f}")
        logging.warning(f"Unrealized P&L: ₹{summary['unrealized_pnl']:.2f}")
        logging.warning(f"Trades Today: {summary['daily_trades']}")

        # Log individual positions with weekly context
        for stock_code, position in summary["positions"].items():
            ...
            if current_price and entry_price:
                ...
                days_held = self.portfolio._calculate_holding_days(
                    position.get("entry_time", "")
                )

                # Determine week and expected performance
                if days_held <= 7:
                    week = "W1"
                    target = 2.5
                elif days_held <= 14:
                ...
                status = "✓" if pnl_pct >= target else "⚠" if pnl_pct >= 0 else "✗"

                logging.warning(
                    f"  {stock_code}: {status} {week} | {pnl_pct:+.1f}% (Target: {target}%)"
                )
    ...
    def generate_daily_report(self):
        ...
        try:
            ...
            summary = self.portfolio.get_portfolio_summary()
            trades = self.portfolio.today_trades

            # Calculate daily P&L
            daily_pnl = sum(trade.get("profit_loss", 0) for trade in trades)

            # Generate report
            report = self._create_daily_report(summary, trades, daily_pnl)

            # Send report
            send_email("Daily Trading Report", report)
    ...
    def _create_daily_report(
        self, summary: dict, trades: list, daily_pnl: float
    ) -> str:
        ...
        # Monthly progress estimation
        current_month_days = datetime.datetime.now().day
        month_progress = (current_month_days / 30.0) * 100
        expected_monthly_pnl = daily_pnl + summary["unrealized_pnl"]
        monthly_target_progress = (expected_monthly_pnl / (INITIAL_CAPITAL * 0.1)) * 100

        report = f"""DAILY TRADING REPORT - {report_date}
=======================================================

MONTHLY PROGRESS (10% Target):
-----------------------------
Current P&L: ₹{daily_pnl + summary['unrealized_pnl']:.2f}
Target: ₹{INITIAL_CAPITAL * 0.1:.2f}
Progress: {monthly_target_progress:.1f}%
Month Progress: {month_progress:.1f}%

PERFORMANCE SUMMARY:
--------------------
Realized P&L Today: ₹{daily_pnl:.2f}
Unrealized P&L: ₹{summary['unrealized_pnl']:.2f}
Total P&L: ₹{daily_pnl + summary['unrealized_pnl']:.2f}
        ...
        if summary["positions"]:
            for stock_code, position in summary["positions"].items():
                ...
                if current_price and entry_price:
                    ...
                    days_held = self.portfolio._calculate_holding_days(entry_time)
                    ...
                    if days_held <= 7:
                        ...
                        week_target = 10.0
                        week = f"Week 4+ ({days_held}d)"

                    status = (
                        "ON TARGET"
                        if pnl_percent >= week_target
                        else "BEHIND" if pnl_percent >= 0 else "LOSS"
                    )
...
{stock_code} - {status}:
    ...
    {week} Target: {week_target}% | Days Held: {days_held}"""
        else:
            report += "\nNo open positions"
        ...
        if trades:
            for i, trade in enumerate(trades, 1):
                ...
                report += f"""
Trade #{i}: {trade.get('stock_code', 'Unknown')} - {result}
    Entry: ₹{trade.get('entry_price', 0):.2f} → Exit: ₹{trade.get('exit_price', 0):.2f}
    P&L: ₹{pnl:.2f} ({pnl_pct:+.2f}%)
    Reason: {trade.get('exit_reason', 'Unknown')}
    Held: {trade.get('holding_days', 0)} days"""
        else:
...
SYSTEM STATUS:
--------------
Paper Trading: {"YES" if PAPER_TRADING_MODE else "NO"}
...
Email Notifications: {"Enabled" if EMAIL_ENABLED else "Disabled"}
    ...
    def _save_report(self, report: str):
        ...
        try:
            ...
            report_date = datetime.datetime.now().strftime("%Y%m%d")
            filename = os.path.join(reports_dir, f"report_{report_date}.txt")
...
def main():
    """Main entry point"""
    try:
        logging.info("=== ENHANCED TRADING SYSTEM STARTING ===")
        ...
        # Run strategy
        success = system.run_strategy()

        # Generate daily report
        system.generate_daily_report()

        logging.info("=== TRADING SYSTEM SESSION COMPLETED ===")
        return success

    except KeyboardInterrupt:
        logging.info("Trading system interrupted by user")
        return True
...
if __name__ == "__main__":
    ...
    logging.info(f"Trading system exiting with code: {exit_code}")
    exit(exit_code)
...
#     # Check if breeze connection exists
#     if breeze is None:
#         logging.error(
#             "Breeze API connection is None. Check if initialize_api() was called successfully."
#         )
#         return None
...
#             # Strategy 1: Check Success list format (most common)
#             if (
...
#                                         price = temp_price
#                                         logging.debug(
#                                             f"Found price {price} for {stock_code} using field '{price_field}'"
#                                         )
#                                         break
...
#                                 price = temp_price
#                                 logging.debug(
#                                     f"Found price {price} for {stock_code} using direct field '{price_field}'"
#                                 )
#                                 break
#                         except (ValueError, TypeError):
#                             continue
...
#                                             price = temp_price
#                                             logging.debug(
#                                                 f"Found price {price} for {stock_code} using legacy field '{price_field}'"
#                                             )
#                                             break
#                                     except (ValueError, TypeError):
#                                         continue
...
#             if not price:
#                 price = _find_price_recursively_minimal(quote)
#                 if price:
#                     logging.debug(
#                         f"Found price {price} for {stock_code} using recursive search"
#                     )
...
# def _find_price_recursively_minimal(data, depth=0, max_depth=3):
#     """Recursively search for price in nested data structures - minimal version"""
...
#         # Check common price field names first
#         price_fields = ["ltp", "last_price", "last", "close", "price"]
#         for field in price_fields:
#             if field in data:
#                 result = _find_price_recursively_minimal(
#                     data[field], depth + 1, max_depth
#                 )
#                 if result:
#                     return result

#         # Then search fields containing price-related keywords
#         for key, value in data.items():
#             if any(keyword in key.lower() for keyword in ["price", "ltp", "last"]):
#                 result = _find_price_recursively_minimal(value, depth + 1, max_depth)
#                 if result:
#                     return result
...
#         for item in data:
#             result = _find_price_recursively_minimal(item, depth + 1, max_depth)
#             if result:
#                 return result
